...
function normalizeCard(raw: Record<string, unknown>): ThaiIdCardPayload {
  return {
    citizenId: String(raw.citizenId ?? raw.cid ?? raw.personalId ?? ""),
    titleTh: raw.titleTh ? String(raw.titleTh) : undefined,
    titleEn: raw.titleEn ? String(raw.titleEn) : undefined,
    fullNameTh: raw.fullNameTh ? String(raw.fullNameTh) : undefined,
    fullNameEn: raw.fullNameEn ? String(raw.fullNameEn) : undefined,
    firstNameTh: raw.firstNameTh ? String(raw.firstNameTh) : undefined,
    firstNameEn: raw.firstNameEn ? String(raw.firstNameEn) : undefined,
    lastNameTh: raw.lastNameTh ? String(raw.lastNameTh) : undefined,
    lastNameEn: raw.lastNameEn ? String(raw.lastNameEn) : undefined,
...
async function startSmartCardReader() {
  await publishReaderStatus("ready", `SmartCard integration point ready; insert delay ${insertCardDelayMs}ms`);
  // Integration point:
  // const { ThaiIdCardReader } = await import("thai-id-card-reader");
  // const reader = new ThaiIdCardReader();
  // reader.onReadComplete((rawCard) => publishCardRead(normalizeCard(rawCard)));
  // reader.onReadError((error) => publishReaderStatus("error", "Read failed"));
}
