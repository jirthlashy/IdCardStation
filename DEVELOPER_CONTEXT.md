...
- Keep tracked developer docs at the repository root by default. Avoid adding nested tracked READMEs under `thai-id-intake/` unless there is a strong reason they cannot live in the root README.
- Operator-facing bundle docs are generated by the release creator into `deploy-transfer*/`. Update the templates in `thai-id-intake/dev-deploy-script/package/lib/CopyDeployFiles.ps1` when scripts, bundle layout, server ports, runtime requirements, or transfer steps change.
